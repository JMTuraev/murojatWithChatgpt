...
export async function GET() {
  try {
    ...
    // 🔁 Endi murojaatlarni tashkilot_id bilan filterlaymiz
    const { data: murojaatlar, error: murojaatError } = await supabaseAdmin
      .from('murojaatlar')
      .select('*')
      .eq('status_id', 2) // biriktirilgan
      .eq('tashkilot_id', profile.id); // aynan ushbu tashkilotga tegishli
